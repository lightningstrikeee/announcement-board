...
# 2) Invalid POST payload should fail validation.
INVALID_POST_BODY="$WORK_DIR/post-invalid.json"
invalid_status="$(request POST "$ANNOUNCEMENTS_URL" '{"title":"x","body":"y","author":"z","pinned":"yes"}' "$INVALID_POST_BODY")"
...
# 3) Valid POST payload should create an announcement.
POST_BODY="$WORK_DIR/post-valid.json"
post_status="$(request POST "$ANNOUNCEMENTS_URL" '{"title":"Smoke Title","body":"Smoke Body","author":"Smoke Tester","pinned":false}' "$POST_BODY")"
...
# 4) GET /announcements/:id should return 200 and the created title.
...
# 5) DELETE /announcements/:id should succeed.
DELETE_BODY="$WORK_DIR/delete.json"
delete_status="$(request DELETE "$ANNOUNCEMENTS_URL/$announcement_id" "" "$DELETE_BODY")"
